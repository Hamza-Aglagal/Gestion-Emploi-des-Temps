...
import GeneratePDFProf from '../components/Pdf/GeneratePdfProf';
...
const AddEmploi = () => {
    ...
    const [
        DataDates, DataProf, DataNiveau, DataFillier, DataGroupes, DataSalleDispo, OneDateSecondaire, DataModules, idProf,
    ...
    const handeSendEmailProf = async () => {
        const pdfData = doc; // Utilisez le PDF généré
        const emailData = {
    ...
    return (
        <Container maxWidth={themeStretch ? false : 'xl'}>
            ...
            <Box sx={{ width: '100%', height: '2.5rem', display: 'flex', justifyContent: 'flex-end', marginTop: '3rem' }} >
                <Button variant="contained" onClick={handeSendEmailProf} > Send Emploi <AttachEmailIcon sx={{ marginLeft: '.5rem' }} /> </Button>
            ...
            <Box>
                {
                    idProf && <GeneratePDFProf IdPROF={idProf} ContentRef={ContentRef} />
                }
            </Box>
