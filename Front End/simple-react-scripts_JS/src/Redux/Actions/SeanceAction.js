...
import {  ALL_SEANCE_OF_PROF, ALL_SEANCE_OF_STUDENT, ALL_SEANCE_RESERVE_OF_PROF, DELETE_SEANCE_TEMPORAIRE, EMPLOI_GROUPES, GET_ERROR_EMPLOI_OF_PROF, GET_ERROR_SEANCE_OF_PROF, MSG_ADD_SEANCE, MSG_ADD_SEANCE_TEMPORAIRE, MSG_DELETE_EMPLOI, MSG_DELETE_SEANCE, TIME_AVAILABLE_PROF_GP } from "../types"
...
// Get All Seance Of Student 
export const GetAllSeanceOfStudent = (idStudent) => async (dispatch) => {
    try {
        const res = await UseGetdata(`/seances/student/${idStudent}`)
        // console.log('res :', res)
        dispatch({ type: ALL_SEANCE_OF_STUDENT, payload: res.data    })
    } catch (error) {
        dispatch({ type: GET_ERROR_SEANCE_OF_PROF, payload:`Error: ${error.response}` })
    }
}
...
// Get All seance temporaire of prof
export const GetAllSeanceTempOfProf = (id) => async (dispatch) => {
    try {
        const res = await UseGetdata(`/seanceTemporaire/prof/${id}`)
        // console.log('res :', res)
        dispatch({ type: ALL_SEANCE_RESERVE_OF_PROF, payload: res.data    })
    } catch (error) {
        dispatch({ type: GET_ERROR_SEANCE_OF_PROF, payload:`Error: ${error.response}` })
    }
}
...
// Delete seance temporaire of prof
export const DeleteSeanceTempOfProf = (id) => async (dispatch) => {
    try {
        const res = await UseGetdata(`/delete-seance/temp/${id}`)
        // console.log('res :', res)
        dispatch({ type: DELETE_SEANCE_TEMPORAIRE, payload: res.data    })
    } catch (error) {
        dispatch({ type: GET_ERROR_SEANCE_OF_PROF, payload:`Error: ${error.response}` })
    }
}