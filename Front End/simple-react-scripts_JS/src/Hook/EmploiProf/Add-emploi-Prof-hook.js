...
import imgTest from '../../assets/img/45578.png'
...
import EmailProf from '../../components/Email/EmailProf'
...
const AddEmploiProfHook = () => {
    ...
    let DataSalleDispo = []
    DataSalleDispo = useSelector(state => state.RedSalle.Salle_Dispo)

    // console.log('data : ', DataSalleDispo)
    ...
    const [EmailOfProf, setEmailProf] = useState(null)
    // console.log('EmailProf :', EmailProf)
    // console.log('idProf :', idProf)
    useEffect(() => {
        if (idProf && DataProf) {
            const filteredProf = DataProf.filter(prof => prof.id === idProf);

            if (filteredProf.length > 0) {
                setEmailProf(filteredProf[0].email);
            } else {
                setEmailProf(null);
            }
        }
